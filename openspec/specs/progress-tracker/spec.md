...
## Purpose
Provides a progress feedback loop for the vocabulary learner: a dashboard streak metric (with one-day grace), two additional header metrics, a "Show Progress" page with a 30-day activity chart, word-level danger signals (struggling and forgetting lists), prod vs recognition averages, theme breakdowns, and duel win/loss summary — all backed by pure, unit-tested stats helpers.
## Requirements
