# vault-filters Specification

## Purpose
Defines the vault temporal filters, in particular that the "Added last X days" filter queries a real per-user `added_at` timestamp on `user_word_settings` (when the user added the word to their library) rather than a global word-creation timestamp.
## Requirements
