...
## Purpose
Defines the behavior of the active practice session: card transitions never reveal the answer side while advancing (solo and duel), the 4-emoji progress pill reflects the current review pass, editing a word mid-session does not error, and attempts persist incrementally in the background.
## Requirements
