# data-access-resilience Specification

## Purpose
Ensure Supabase query failures (including RLS denials, which return empty data with a null error
on reads but populate `error` on writes) are detected and surfaced to the user via toast and logged,
rather than silently rendering as empty or incomplete UI.
## Requirements
