# word-creation-integrity Specification

## Purpose
Guarantee that creating a new word is all-or-nothing: the shared `words_dim` row and the author's
`user_word_settings` row are written within a single transaction, so a partial failure never leaves
an orphaned word with no owning settings row.
## Requirements
